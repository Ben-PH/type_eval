...
pub fn write_op_macro() -> ::std::io::Result<()> {
    ...
    write!(
        ...
        "
/**
Credit goes to `github:paholg/typenum` for original macro work:

https://github.com/paholg/typenum/blob/b237efbb1da9646435274f63d7290cb7651b031f/build/op.rs
...
For example, `U5` is okay, but `type_eval::U5` is not.
...
// ```rust
// #![recursion_limit=\"128\"]
// #[macro_use] extern crate type_eval;
// use type_eval::num_vals::*;
//
// fn main() {{
//     assert_type!(
//         op!(min((U2 - U1) * (U3 + N7), U5 * (U3 + U4)) == U10)
//     );
// }}
// ```
...
(@eval @stack[$a:ty, $b:ty, $($stack:ty,)*] @input[{op}, $($tail:ident,)*]) => (
    __op_internal__!(@eval @stack[$crate::prelude::{op}<$b, $a>, $($stack,)*] @input[$($tail,)*])
);",
            op = op.operator
        )?;
    }
...
(@eval @stack[$a:ty, $($stack:ty,)*] @input[{op}, $($tail:ident,)*]) => (
    __op_internal__!(@eval @stack[$crate::prelude::{op}<$a>, $($stack,)*] @input[$($tail,)*])
);",
            op = op.operator
        )?;
    }
